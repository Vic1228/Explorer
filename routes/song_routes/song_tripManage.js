...
song_tripManage_router.get("/", function (req, res) {
  ...
  var data = {
    ...
    sharedItems: [
      {
        sharedItemName: "",
        provideList: [{ userId: "", userName: "", itemCount: "" }],
      },
    ],
    privateItems: [{ privateItemName: "", itemCount: "" }],
    schedule: [
      {
        day: "",
        activity: [{ startTime: "", activityName: "" }],
      },
    ],
    tripNotes: "",
  };
  ...
  conn
    .queryAsync(
      `SELECT tripId, positionState FROM tripmembers WHERE userId = ${userId} ORDER BY tripId , positionState DESC`
    )
    .then((result) => {
      // function
      result.forEach((item) => {
      ...
      return conn.queryAsync(
        `SELECT tripName, positionState FROM tripmembers WHERE userId = ${userId} ORDER BY positionState`
      );
    })
    .then((bee) => {
      // console.log(apple);
